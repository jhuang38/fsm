use std::{borrow::Borrow, fmt::format, path::Path, sync::{mpsc::{self, Sender}, Arc, Mutex}, thread, time::Duration};
...
impl FileSweepManager {
    pub fn new(repeat_duration: Duration) -> Self {
        Self{repeat_duration: Arc::new(Mutex::new(repeat_duration)), sender: None, thread_handle: None}
    }
        ...
        let handle = thread::spawn(move || -> Result<(), FsmError> {
            ...
            while !done {
                ...
                if !path_to_watch.as_ref().exists() {
                    return Err(FsmError::new(ErrorType::SweepError, format!("The path {:#?} does not exist.", path_to_watch.as_ref())));
                }
                if !path_to_watch.as_ref().is_dir() {
                    return Err(FsmError::new(ErrorType::SweepError, format!("The path {:#?} is not a directory.", path_to_watch.as_ref())));
                }
                ...
                info!("Found {:#?} files in {:?}, reorganizing.", count, path_to_watch.as_ref());
