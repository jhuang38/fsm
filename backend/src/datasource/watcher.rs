...
use crate::{config::ConfigManager, error::{ErrorType, FsmError}, filepath::{self, FilepathManager}, filter::FilterManager};

extern crate notify;
extern crate notify_debouncer_full;

pub struct DirectoryWatcher {
    debounced_watcher: notify_debouncer_full::Debouncer<notify::ReadDirectoryChangesWatcher, notify_debouncer_full::FileIdMap>
}

impl DirectoryWatcher {
    pub fn new(filter_manager: Arc<Mutex<FilterManager>>, filepath_manager: Arc<Mutex<FilepathManager>>, config_manager: Arc<Mutex<ConfigManager>>) -> Result<Self, FsmError> {
                ...
                Ok(events) => {
                    ...
                    events.iter().for_each(|event| {
                        ...
                        match filepath_manager.lock() {
                            Ok(mutex_guard) => {
                                let _ = (*filter_manager).place_file_in_mapped_location(filepath, &*mutex_guard);
                            },
                            Err(_) => {}
                        }
        ...
        if debouncer.is_err() {
            return Err(FsmError::new(ErrorType::WatcherError, debouncer.unwrap_err().to_string()));
        }
        let mut debouncer: notify_debouncer_full::Debouncer<notify::ReadDirectoryChangesWatcher, notify_debouncer_full::FileIdMap> = debouncer.unwrap();
        ...
        Ok(Self { debounced_watcher: debouncer })
